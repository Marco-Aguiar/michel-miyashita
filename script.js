...
trial?.addEventListener('submit', (e)=>{
  ...
  const nome = fd.get('nome'); const tel = fd.get('telefone');
  const periodo = fd.get('periodo'); const nivel = fd.get('nivel');
  const extra = fd.get('mensagem') || '';
  const msg = encodeURIComponent(`Olá! Sou ${nome}. Tel: ${tel}. Quero aula experimental (${nivel}) no período ${periodo}. ${extra ? 'Obs: '+extra : ''}`);
